    ...
    public function execute() {
        try {
            if (request::getInstance()->isMethod('POST') and request::getInstance()->isAjaxRequest()) {
                ...
                detalleHojaTableClass::delete($ids, true); /* el true es para el borrado logico  en bd false si no lo tiene */
                
                $this->arrayAjax = array(
                    'code' => 200,
                    'msg' => 'la eliminacion fue exitosa'
                );
                $this->defineView('delete', 'detalleHoja', session::getInstance()->getFormatOutput());
                session::getInstance()->setSuccess('el registro se elimino con exito'); /* mensaje de exito */
//                 log::register('eliminar',  detalleHojaTableClass::getNameTable());//linea de bitacora
            } else {
