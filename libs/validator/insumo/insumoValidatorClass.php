...
//      } 
            ...
            $fechaCreacion = request::getInstance()->getPost(\insumoTableClass::getNameField(\insumoTableClass::FECHA_FABRICACION, true));
            $fechaVencimiento = request::getInstance()->getPost(\insumoTableClass::getNameField(\insumoTableClass::FECHA_VENCIMIENTO, true));
            if (strtotime($fechaVencimiento) < strtotime($fechaCreacion)) {
                $flag = true;
                session::getInstance()->setFlash('inputFechaVencimiento', true); /* input usuario biene del formulario */
                session::getInstance()->setError('el campo fecha vencimiento no puede ser mayor al de fabricacion', 'inputFechaVencimiento');
            }
