...
//      $soloNumeros = "/^[[:digit:]]+$/";
        ...
        public static function validateFiltroTipo($descripcion) {
            $soloLetras = "/^[a-z]+$/i";
            $soloTelefono = "/^(\d{3,3}\-\d{3,3}\-\d{4,4})|^(\+\d\-\d{3,3}\-\d{4,4})/";
            $emailcorrecto = '/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/';


             if (strlen($descripcion) > \tipovTableClass::DESC_TIPOV_LENGTH) {
//                 echo $descripcion;
//                 exit();
                session::getInstance()->setError('el tipo venta digitado es mayor en cantidad de caracteres a lo permitido', 'inputDescTipoV');
            }
//            //----solo permitir letras----
               ...
               session::getInstance()->setError('El campo descripcion tipo venta no permite numeros, solo letras', 'inputDescTipoV');
            }
        }
